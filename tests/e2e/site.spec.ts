...
for (const path of ['/', '/demo/', '/privacy/', '/terms/', '/404.html']) {
  test(`${path} has semantic structure and no serious axe violations`, async ({ page }) => {
    ...
    await expect(page).toHaveTitle(/Pronunciation Cards|Privacy|Terms|Page not found/);
...
test('home works at 390px and gives the sample as the first action', async ({ page }) => {
  const runtimeRequests: string[] = [];
  page.on('request', (request) => runtimeRequests.push(request.url()));
  await page.setViewportSize({ width: 390, height: 844 });
  await page.goto('/');
  await expect(page.getByRole('heading', { level: 1 })).toContainText('Save pronunciations for technical words');
  await expect(page.getByText('For blind and low-vision screen-reader users')).toBeVisible();
  await expect(page.getByRole('link', { name: 'Try it with sample data' })).toHaveAttribute('href', '/demo/');
  await expect(page.getByRole('link', { name: /Download the extension ZIP/ })).toHaveAttribute('download', '');
  await expect(page.locator('a[href="/downloads/pronunciation-cards-chrome.zip"]')).toHaveCount(2);
  ...
  for (const target of [
    ...
    page.getByRole('link', { name: 'Demo' }).last(),
  ]) {
...
test('controlled clients bypass stale page and download cache entries', async ({ page }) => {
  ...
  await page.evaluate(async () => {
    const cache = await caches.open('pronunciation-cards-site-v5');
